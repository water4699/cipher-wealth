...
const CONTRACT_NAME = "CipherWealth";
...
// Read from artifacts if deployments don't exist yet
function readFromArtifacts(contractName) {
  const artifactPath = path.join(dir, "artifacts", "contracts", `${contractName}.sol`, `${contractName}.json`);
  
  if (!fs.existsSync(artifactPath)) {
    console.error(`${line}Unable to locate artifact at ${artifactPath}${line}`);
    process.exit(1);
  }
...
// Try to read deployments, fallback to artifacts
let deployLocalhost = readDeployment("localhost", 31337, CONTRACT_NAME, true);
if (!deployLocalhost) {
  console.log("No localhost deployment found, using artifact ABI with placeholder address");
  deployLocalhost = readFromArtifacts(CONTRACT_NAME);
}

let deploySepolia = readDeployment("sepolia", 11155111, CONTRACT_NAME, true);
if (!deploySepolia) {
  console.log("No Sepolia deployment found, using artifact ABI with placeholder address");
...
*/
export const ${CONTRACT_NAME} = ${JSON.stringify({ abi: deployLocalhost.abi }, null, 2)} as const;
\n`;
...
console.log(`Generated ${path.join(outdir, `${CONTRACT_NAME}.ts`)}`);
console.log(`Generated ${path.join(outdir, `${CONTRACT_NAME}Addresses.ts`)}`);
console.log(tsAddresses);

fs.writeFileSync(path.join(outdir, `${CONTRACT_NAME}.ts`), tsCode, "utf-8");
fs.writeFileSync(
